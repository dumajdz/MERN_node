...
const createProduct = (newProduct) => {                                                                                                                                     
    return new Promise(async (resolve, reject) => {
        ...
        try {
            //kiem tra san pham co hay khong
...
const getAllProduct = (limit, page, sort, filter) => {
    return new Promise(async (resolve, reject) => {
        try {
            ...
            if(!limit) {
                allProduct = await Product.find().sort({createdAt: -1, updatedAt: -1})// truy van all san pham va sap xep theo time toa va time update
            }else {
...
const getAllType = () => {
    return new Promise(async (resolve, reject) => {
        try {
            const allType = await Product.distinct('type')//lay tat ca cac gia tri duy nhat o truong Type
            resolve({
                status: 'OK',
                message: 'Success',
                data: allType,
